...
1. Для начала изучили материал лабораторной работы. Далее на основе примеров напишем аналогичные программы, но с изменениями.

2. Создадим необходимые файлы для работы. (рис. [-@fig:001])

![Создание файлов](image/Рис.1.png){ #fig:001 width=70% }

3. Затем изменим коды программ, данных в лабораторной работе. В файл common.h добавим стандартные заголовочные файлы: "unistd.h", "time.h". Это необходимо для работы других файлов. Этот файл является заголовочным, чтобы в остальных програмах не прописывать одно и то же каждый раз. (рис. [-@fig:002])

![Файл common.h](image/Рис.2.png){ #fig:002 width=70% }

4. Затем в файл server.c добавляем цикл "while" для контроля за верменем работы сервера. Разница между текущим временем и началом работы не должна превышать 30 секунд. (рис. [-@fig:003], [-@fig:004])

![Файл server.c](image/Рис.3.png){ #fig:003 width=70% }

![Файл server.c](image/Рис.4.png){ #fig:004 width=70% }

5. В файл client.c добавим цикл, который отвечает за количество сообщений о текущем времени (4 сообщения). С промощью команды "sleep" приостановим работу клиента на 5 секунд. (рис. [-@fig:005], [-@fig:006])

![Файл client.c](image/Рис.5.png){ #fig:005 width=70% }

![Файл client.c](image/Рис.6.png){ #fig:006 width=70% }

6. Makefile оставили без изменений. (рис. [-@fig:007])

![Файл Makefile](image/Рис.7.png){ #fig:007 width=70% }

7. Далее делаем компиляцию файлов с помощью команды "make all". (рис. [-@fig:008])

![Компиляция](image/Рис.8.png){ #fig:008 width=70% }

8. Затем открываем три терминала для проверки работы наших файлов. В первом пишем "./server", а в остальных "./client". В результате каждый терминал вывел по 4 сообщения, а по истечение 30 секунд работа сервера была завершена. Всё работает верно. (рис. [-@fig:009])

![Проверка](image/Рис.9.png){ #fig:009 width=70% }

9. Проверим длительность работы сервера. Вводим команду "./server" в одном терминале. Он завершил свою раюоту через 30 секунд. Если сервер завершит свою работу, не закрыва канал, то при повторном запуске появится ошибка "Невозможно создать FIFO", так как у нас уже есть один канал. (рис. [-@fig:010])

![Проверка](image/Рис.10.png){ #fig:010 width=70% }
